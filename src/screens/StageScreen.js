// src/screens/StageScreen.js — 2/3-BOSQICH: Davom etmoqda / Yakunlandi
...
export default function StageScreen({ navigation, route }) {
  const { hisobot } = route.params; // meniki ro'yxatidan kelgan hisobot obyekti
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status !== 'granted') { setGpsXato('Lokatsiya ruxsati berilmadi'); return; }
      try {
        const loc = await Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.High });
        setGps({ lat: loc.coords.latitude, lng: loc.coords.longitude, mocked: loc.mocked === true });
      } catch { setGpsXato('GPS aniqlanmadi'); }
    })();
  }, []);
  ...
  async function yubor() {
    ...
    try {
      if (net.isConnected) {
        ...
        if (res.ok) {
          Alert.alert('Qabul qilindi ✓',
            keyingiBosqich === 'davom' ? "2-bosqich yuborildi. Ish tugagach 'Yakunlandi' bosqichini yuklang." : 'Ish to\'liq yakunlandi!',
            [{ text: 'OK', onPress: () => navigation.navigate('Home') }]);
        } else Alert.alert('Rad etildi', res.xato);
      } else {
        await navbatgaQosh({ ...payload, action });
        Alert.alert('Offline saqlandi', "Internet kelganda avtomatik yuboriladi.", [{ text: 'OK', onPress: () => navigation.navigate('Home') }]);
      }
    } catch (e) {
      await navbatgaQosh({ ...payload, action });
      Alert.alert('Navbatga saqlandi', '', [{ text: 'OK', onPress: () => navigation.navigate('Home') }]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: RANG.fon }}>
      <View style={s.header}>
        ...
        <Text style={s.title}>{sarlavha}</Text>
        <View style={{ width: 60 }} />
      </View>

      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        <View style={s.ishCard}>
          <Text style={s.ishNomi}>{hisobot.ishNomi || hisobot.ishTuri}</Text>
          <Text style={s.ishBoshTavsif} numberOfLines={2}>{hisobot.b_tavsif}</Text>
        </View>
        ...
        <Text style={s.label}>{keyingiBosqich === 'davom' ? 'Hozirgi holat tavsifi *' : 'Yakuniy natija tavsifi *'}</Text>
        <TextInput style={[s.input, { height: 110, textAlignVertical: 'top' }]} value={tavsif}
          onChangeText={setTavsif} multiline
          placeholder={keyingiBosqich === 'davom' ? 'Ish qay darajada bajarildi...' : 'Yakuniy natija, umumiy xulosa...'} />

        <Text style={s.label}>Rasmlar * (1–5 ta)</Text>
        ...
        <TouchableOpacity style={[s.yuborBtn, yuborilmoqda && { opacity: 0.6 }]} onPress={yubor} disabled={yuborilmoqda}>
          {yuborilmoqda ? <ActivityIndicator color="#fff" /> : <Text style={s.yuborText}>{keyingiBosqich.toUpperCase()}NI YUBORISH</Text>}
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
...
const s = StyleSheet.create({
  ...
  title: { fontSize: 15, fontWeight: '800', color: RANG.toq },
  ishCard: { backgroundColor: RANG.oq, borderRadius: 12, padding: 14, marginBottom: 12, borderWidth: 1, borderColor: RANG.chiziq },
  ishNomi: { fontWeight: '800', fontSize: 15, color: RANG.toq },
  ishBoshTavsif: { color: RANG.kul, fontSize: 12.5, marginTop: 4 },
